...
description: 本文整理 Claude Code CLI 常用命令、快捷键与使用说明。
---

> 本文档整理自 Claude Code CLI 的 `/help` 界面，
> 涵盖 **General（通用）**、**Commands（内置命令）** 和 **Custom Commands（自定义命令）** 三个部分。
...
| `/claude-api` | 加载 Claude API 参考文档，导入 `anthropic` 时自动触发 |
